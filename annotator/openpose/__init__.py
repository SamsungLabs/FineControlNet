...
def draw_pose(pose, H, W, draw_body=True, draw_hand=True, draw_face=True, given_canvas=None, color=None):
    bodies = pose['bodies']
    faces = pose['faces']
    hands = pose['hands']
    candidate = bodies['candidate']
    subset = bodies['subset']
    canvas = np.zeros(shape=(H, W, 3), dtype=np.uint8) if given_canvas is None else given_canvas

    if draw_body:
        canvas = util.draw_bodypose(canvas, candidate, subset, color)
