...
def parse_args():
    ...
    parser.add_argument('--save_dir', type=str, default='./test_outputs/mlsd/')
    parser.add_argument('--img_path', type=str, default='./test_imgs/example_mlsd/', help='Direcotry to source images or MLSD edges.')
    ...
    parser.add_argument('--instance_descriptions', nargs='*', default=["an old bookshelf", "a wood desk"], help='List of N instance descriptions.')
    parser.add_argument('--setting_descriptions', type=str, default='in a medieval castle', help='Description of the setting.')
    parser.add_argument('--xy', nargs='*', type=lambda s: tuple(map(int, s.split(','))), default=[(30,80), (250,270)], help='List of x,y locations as tuples (x,y).')
    parser.add_argument('--wh', nargs='*', type=lambda s: tuple(map(int, s.split(','))), default=[(170,320), (225,125)], help='List of width,height dimensions as tuples (w,h).')
...
if __name__ == '__main__':
    ...
    n_prompt = 'low resolution, blur, noise, bad anatomy, cropped, worst quality'  # "Negative Prompt"
    num_samples = 1 # 50
    guess_mode = False
    strength = 1.5
    scale = 15  # guidance scale # max 30,  min 0.1
    eta = 0.0
